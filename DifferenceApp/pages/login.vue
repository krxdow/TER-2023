<template>
  <div
    class="flex flex-column align-items-center justify-content-center h-screen"
  >
    <div
      class="surface-card p-4 shadow-2 border-round w-full lg:w-6 flex flex-column justify-content-center"
    >
      ...
      <div v-if="!show" class="text-center mb-5">
        ...
        <span
          class="text-600 font-medium line-height-3 font-medium no-underline ml-2 text-blue-500 cursor-pointer"
          @click="
            show = !show;
            udpateLabel(show);
          "
      ...
      <div v-if="show">
        <Button
          ...
          @click="
            signIn('credentials', {
              username,
              password,
              callbackUrl: originalProtectedPagePatch,
            })
          "
        ></Button>
        ...
        <Button
          ...
          @click="signIn('github', { callbackUrl: originalProtectedPagePatch })"
        ></Button>
      </div>
      ...
      <div v-if="!show">
        <Button
          ...
          label="Register"
          type="submit"
          @click="addUsers(username, email, password)"
        >
        </Button>
      </div>
    </div>
...
async function udpateLabel(show) {
  usernameLabel.value = show
    ? "Email or Username"
    : "Entrer un Nom Utilisateur";
